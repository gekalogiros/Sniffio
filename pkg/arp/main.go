...
import (
	...
	"github.com/gekalogiros/sniffio/pkg/utilities"
)
...
// Sniff Records arp traffic using the en0 interface
...
`
			...
			sourceIP := utilities.IP(arp.SourceProtAddress)
			sourceMac := utilities.Mac(arp.SourceHwAddress)
			sourceVendor := macLookup.LookupVendor(strings.ToUpper(sourceMac))
			destinationIP := utilities.IP(arp.DstProtAddress)
			destinationMac := utilities.Mac(arp.DstHwAddress)
...
// Detect Locates devices using arp requests
